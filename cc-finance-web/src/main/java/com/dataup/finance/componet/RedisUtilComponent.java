...
public class RedisUtilComponent {
	...
	public Jedis getRedisInstance() throws BusinessException{
		...
		try {
			...
			if(e instanceof BusinessException){
				...
				throw new BusinessException(BusinessCode.REDIS_EXCEPTION+"",PropertiesConfig.getProperty(BusinessCode.REDIS_EXCEPTION+""));
			}
		}
	}
	...
	public <T> T getRedisJsonCache(String key,Class<T> clazz,JsonBinder binder,JavaType javaType)throws BusinessException{
		Jedis jedis=null;
		try {
			jedis=this.getRedisInstance();
			String value=jedis.get(key);
	...
	public void lockIdByRedis(String prefix,String id,  int expire,Jedis jedis) throws BusinessException {
		if(jedis==null){
    		throw new BusinessException(BusinessCode.REDIS_EXCEPTION+"",PropertiesConfig.getProperty(BusinessCode.REDIS_EXCEPTION+""));
    	}
		Long returnValue = 1L;
		try {
			...
			if (returnValue == 0) {
				...
				throw new BusinessException(BusinessCode.LOCK_BY＿REDIS+"",PropertiesConfig.getProperty(BusinessCode.LOCK_BY＿REDIS+""));
			}
	...
	public void lockIdByRedis(String prefix,String id, int expire) throws BusinessException {
		Jedis jedis=null;
		Long returnValue = 1L;
		try {
			jedis=this.getRedisInstance();
